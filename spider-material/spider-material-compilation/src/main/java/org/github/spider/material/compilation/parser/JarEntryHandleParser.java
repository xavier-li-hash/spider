package org.github.spider.material.compilation.parser;
...
import org.github.spider.material.compilation.dto.classes.InnerClassInfo;
...
import org.github.spider.material.compilation.handler.ClassHandler;
import org.github.spider.material.compilation.spring.UseSpringBeanByAnnotationHandler;
import org.github.spider.material.compilation.util.JavaCGByteCodeUtil;
import org.github.spider.material.compilation.util.JavaCGClassMethodUtil;
import org.github.spider.material.compilation.util.JavaCGSignatureUtil;
...
public class JarEntryHandleParser extends AbstractJarEntryParser {
    ...
    protected boolean handleEntry(ZipInputStream zipInputStream, String jarEntryPath) throws IOException {
        ...
        if (!FileUtils.isClassFile(jarEntryPath)) {
            // 非class文件则跳过
            return true;
        }
    ...
    private void tryRecordJarInfo() throws IOException {
        ...
        if (recordedJarNum.add(lastJarNum)) {
            /*
                ...
                向文件写入数据，内容为jar包信息
             */
            FileUtils.write2FileWithTab(jarInfoWriter, lastJarInfo.getJarType(), String.valueOf(lastJarNum), lastJarInfo.getJarPath());
        }
    }
    ...
    private boolean handleJavaClass(JavaClass javaClass, String jarEntryPath) throws IOException {
        ...
        // 记录类的信息
        FileUtils.write2FileWithTab(classInfoWriter, className, String.valueOf(javaClass.getAccessFlags()), classMd5, classJarNum);
    ...
    private void recordExtendsAndImplInfo(JavaClass javaClass, String className) throws IOException {
        ...
        if (!JavaCGClassMethodUtil.isObjectClass(superClassName)) {
            // 仅处理父类非Object类的情况
            FileUtils.write2FileWithTab(extendsImplWriter,
                    className,
                    accessFlagsStr,
    ...
    private void handleInnerClass(JavaClass javaClass) throws IOException {
        ...
        for (InnerClassInfo innerClassInfo : innerClassInfoList) {
            FileUtils.write2FileWithTab(innerClassWriter, innerClassInfo.getInnerClassName(), innerClassInfo.getOuterClassName(),
    ...
    private void recordSignatureArgumentInfo(String className, String type, String superOrInterfaceName, SignatureAttribute.ClassType classType) throws IOException {
        ...
        for (SignatureAttribute.TypeArgument typeArgument : arguments) {
            ...
            if (objectType instanceof SignatureAttribute.ClassType) {
                ...
                FileUtils.write2FileWithTab(classSignatureEI1Writer, className, type, superOrInterfaceName, String.valueOf(seq),
