package org.github.spider.material.compilation.conf;
...
import org.github.spider.material.compilation.exceptions.JavaCGError;
...
public class JavaCGConfigureWrapper {
    ...
    public Set<String> getOtherConfigSet(JavaCGOtherConfigFileUseSetEnum javaCGOtherConfigFileUseSetEnum, boolean printLog) {
        ...
        // 获取其他配置文件中的参数
        configSet = FileUtils.readFile2Set(JavaCGConfManager.getInputRootPath() + configFileName);
        if (printLog) {
    ...
    public List<String> getOtherConfigList(JavaCGOtherConfigFileUseListEnum javaCGOtherConfigFileUseListEnum, boolean printLog) {
        ...
        // 获取其他配置文件中的参数
        configList = FileUtils.readFile2List(JavaCGConfManager.getInputRootPath() + configFileName);
        if (printLog) {
            logger.info("使用配置文件中的参数 [{}]\n{}", configFileName, StringUtils.join(configList, " "));
        }
        return configList;
