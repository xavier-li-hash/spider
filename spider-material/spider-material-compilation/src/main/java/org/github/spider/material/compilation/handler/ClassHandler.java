...
import org.github.spider.material.compilation.bytecode.BadBytecode;
import org.github.spider.material.compilation.bytecode.SignatureAttribute;
...
import org.github.spider.material.compilation.dto.field.FieldPossibleTypes;
...
public class ClassHandler {
    ...
    private void recordReferencedClass() throws IOException {
        ...
        // 首先写入当前类的类名
        FileUtils.write2FileWithTab(classNameWriter, JavaCGConstants.FLAG_HASHTAG);
        FileUtils.write2FileWithTab(classNameWriter, className);
        ...
        for (String referencedClass : referencedClassList) {
            ...
            FileUtils.write2FileWithTab(classNameWriter, referencedClass);
        }
    }
    ...
    private void handleField(Field field,
        ...
        // 记录字段信息
        FileUtils.write2FileWithTab(fieldInfoWriter, className, fieldName, fieldTypeStr, fieldModifiers, primitiveType, staticFlag, finalFlag,genericSignature);
    ...
    private boolean handleMethod(Method method,
        ...
        if (handledMethodNameAndArgs.add(methodNameAndArgs)) {
            ...
            // 记录方法的信息
            FileUtils.write2FileWithTab(methodInfoWriter, fullMethod, String.valueOf(method.getAccessFlags()), returnType, methodHash, classJarNum);
        } else {
            // 出现方法名+参数类型均相同的方法
    ...
    private void recordMethodReturnGenericsType(String fullMethod, SignatureAttribute.MethodSignature methodSignature) {
        try {
            ...
            // 记录返回类型，序号固定为0
            FileUtils.write2FileWithTab(methodReturnGenericsTypeWriter, fullMethod, JavaCGConstants.FILE_KEY_CLASS_TYPE, "0",
                    JavaCGSignatureUtil.getClassName(classType));

            // 获取到方法返回中泛型类型，记录
            for (int i = 0; i < methodReturnGenericsTypeList.size(); i++) {
                FileUtils.write2FileWithTab(methodReturnGenericsTypeWriter, fullMethod, JavaCGConstants.FILE_KEY_GENERICS_TYPE, String.valueOf(i),
    ...
    private void recordMethodArgsGenericsType(String fullMethod, SignatureAttribute.MethodSignature methodSignature) throws IOException {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            // 记录参数类型，序号固定为0
            FileUtils.write2FileWithTab(methodArgGenericsTypeWriter, fullMethod, String.valueOf(i), JavaCGConstants.FILE_KEY_CLASS_TYPE, "0",
                    JavaCGSignatureUtil.getClassName(classType));

            // 获取到方法参数中泛型类型，记录
            for (int j = 0; j < methodArgsGenericsTypeList.size(); j++) {
                FileUtils.write2FileWithTab(methodArgGenericsTypeWriter, fullMethod, String.valueOf(i), JavaCGConstants.FILE_KEY_GENERICS_TYPE,
    ...
    private void recordMethodArgument(String fullMethod, MethodGen mg) throws IOException {
        ...
        for (Attribute attribute : mg.getAttributes()) {
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                FileUtils.write2FileWithTab(methodArgumentWriter, fullMethod, String.valueOf(i), argTypes[i].toString(), argName);
            }
            return;
        }
